...
Several clusters seem to correspond to fibroblasts. We could use our threshold filtering to select these cells or we could instead use point and click selection using the *Freehand Selection* tool. Lets start by grabbing one of these clusters. 
...
As you may have noticed there is also a dedicated pane to filtering and slecting cells based on various criteria: expression level, cluster identity etc. 
...
A lot of the time there are issues with the QC that need to be remedied. Most of the time this will require taking the data into R/Python to use a collection of scRNAseq tools. But Loupe Browser does allow for some optimzxation with out leaving the GUI. 

The *Reanlyze* tab allows you access to custom filtering of low quality cells, clustering and dimension reduction. 
...
We often start with a 10% cut-off (1% for snRNAseq), but if you are seeing a lot of ambientRNA you may have to be flexible around this. 
...
You can customize some of the options used in the dimension reduction by adjusting the parameters. This allows you to control PCA, tSNE and UMAP. Generally though it is best to stick to the defualt unless you know what you're doing. 
